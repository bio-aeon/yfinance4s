...
/** A scheduled earnings event from Yahoo Finance's market-wide calendar feed.
  *
  * Returned by [[io.github.coinductive.yfinance4s.Calendars.getEarningsCalendar]]. Unlike [[EarningsDate]], this
  * carries company and market-cap metadata plus a scheduling-timing modifier. Unlike [[EarningsHistory]] (which comes
  * from the analyst-trend API and uses quarter labels), this has an explicit `ZonedDateTime` and covers future events.
  *
  * @param symbol
